...
class SpaceCraft:
    def __init__(self, 
        ...
        self.ThrustVectorMatrix: np.array = np.zeros([len(self.thrusters),6]) # thrust and torque generated by each thruster
        self.thruster_states_dic: np.dict = {}

        if I.shape != (3,3):
            raise ValueError("I must have shape (3, 3)")
        
        ######################################################################
        # This code computes the thrust and torque generated by each thruster
        # ---> ThrustVectorMatrix <---
        ###################################################################### 
        for ii in range(0,len(self.thrusters)):
            ...
            Fx_ii, Fy_ii, Fz_ii = T_ii.F_max*-unit_vector # NB! the thrust is in the opposite direction to the unit vector plot (hence minus sign)
            # to handle computer zero
            if abs(Fx_ii)<1e-8: Fx_ii = 0
            if abs(Fy_ii)<1e-8: Fy_ii = 0
            if abs(Fz_ii)<1e-8: Fz_ii = 0;
        ...
        ####################################################################
        # This code computes the required thruster configuration for a single thrust / torque command!
        # It uses a brute force method where it tries all combinations of thrusters, it selects the minimum number of thrusters for a desired actuation
        # ---> thruster_states_dic <--- is a dictionary and should be called using the desired "Actuator_heading" 
        ####################################################################
        num_thrusters = self.ThrustVectorMatrix.shape[0]
        Actuator_headings = ["Fx","Fy","Fz","Tx","Ty","Tz","-Fx","-Fy","-Fz","-Tx","-Ty","-Tz"]
        
        # for positive thrusts/torques
        for ii in range(0, 6): # ii for Fx,Fy,Fz,Tx,Ty,Tz respectively
            poss_thruster_states = np.zeros([1000,num_thrusters])
            count = 0
            # Try increasing numbers of thrusters, starting with the smallest sets
            print("******************NEW THRUST DIRECTION CALC *********************")   
            for r in range(1, num_thrusters + 1):
                # Go through all combinations of thrusters for this number of total possible thrusters
                for thruster_indices in combinations(range(num_thrusters), r):
                    ...
                    resultant_force_torque = np.sum(self.ThrustVectorMatrix * thruster_states_ii[:, None], axis=0) # Fx, Fy, Fz, τx, τy, τz
                    # Check if it meets the criteria
                    if resultant_force_torque[ii]>0 and np.all(abs(resultant_force_torque[np.arange(len(resultant_force_torque)) != ii]) < 1e-8):
                        poss_thruster_states[count,:] = thruster_states_ii
                        print(thruster_states_ii)
                        print(resultant_force_torque)
                        print(resultant_force_torque[np.arange(len(resultant_force_torque)) != ii])
                        count = count+1
            # here, all the thruster combinations for all number of thrusters have been checked. Now we calc config with fewest thrusters
            # Count number of ones in each row
            ones_count = np.sum(poss_thruster_states, axis=1)
            print(poss_thruster_states)
            # Check if all rows are zero
            if np.all(ones_count == 0):
                thruster_config_ii = np.zeros(poss_thruster_states.shape[1], dtype=int) # set the thruster config to zero
            else:
                # Find the index of the row with the fewest ones
                ones_count = ones_count[ones_count != 0]
                min_ones_row_index = np.argmin(ones_count)
                thruster_config_ii = poss_thruster_states[min_ones_row_index,:]
            self.thruster_states_dic[Actuator_headings[ii]] = thruster_config_ii # assign thruster config to dictionary
        ...
        for ii in range(0, 6):
            ...
            # Try increasing numbers of thrusters, starting with the smallest sets
            for r in range(1, num_thrusters + 1):
                for thruster_indices in combinations(range(num_thrusters), r): # all the combinations of thrusters for the given number of total thrustesrs
                    thruster_states_ii = np.zeros(num_thrusters)  # All off
                    thruster_states_ii[list(thruster_indices)] = 1  # Turn on selected thrusters
                    # Compute total force & torque
                    resultant_force_torque = np.sum(self.ThrustVectorMatrix * thruster_states_ii[:, None], axis=0) # Fx, Fy, Fz, τx, τy, τz
            ...
            if np.all(ones_count == 0):
                ...
                min_ones_row_index = np.argmin(ones_count)
                thruster_config_ii = poss_thruster_states[min_ones_row_index,:]
            self.thruster_states_dic[Actuator_headings[ii+6]] = thruster_config_ii # assign thruster config to dictionary
    ...
    def DrawSpaceCraft(self, DRAW_THRUSTERS=0):
        ...
        # Add faces to plot
        ax.add_collection3d(Poly3DCollection(faces, facecolors='orange', linewidths=1, edgecolors='black', alpha=0.3))
