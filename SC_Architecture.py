...
class SpaceCraft:
    def __init__(self, 
        ...
        # Calculate the thrust and torque generated by each thruster
        ...
        # It uses a brute force method where tries all combinations of thrusters
            ...
            for r in range(1, num_thrusters + 1):
                for thruster_indices in combinations(range(num_thrusters), r):
                    ...
                    if COMPLETE == 0 and resultant_force_torque[ii]>0 and np.sum(resultant_force_torque)-resultant_force_torque[ii] == 0:
                        self.thruster_states_dic[Actuator_headings[ii]] = thruster_states_ii
                        COMPLETE = 1 # we have found the correct thruster config for this loop
            if COMPLETE == 0:    
                self.thruster_states_dic[Actuator_headings[ii]] = np.zeros(num_thrusters) # all zeros no combination works!
        ...
        for ii in range(0, 6):
            ...
            for r in range(1, num_thrusters + 1):
                for thruster_indices in combinations(range(num_thrusters), r):
                    ...
                    # Check if it meets the criteria
                    if COMPLETE == 0 and resultant_force_torque[ii]<0 and np.sum(resultant_force_torque)-resultant_force_torque[ii] == 0:
                        self.thruster_states_dic[Actuator_headings[6+ii]] = thruster_states_ii
                        COMPLETE = 1
            if COMPLETE == 0:    
                self.thruster_states_dic[Actuator_headings[6+ii]] = np.zeros(num_thrusters) # all zeros no combination works!
